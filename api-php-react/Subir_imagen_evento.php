...
$response = array();
$upload_dir = $_SERVER['DOCUMENT_ROOT']."/EnsoLearningBackend/Archivos_u/eventos/";
$server_url = $_servidor."/EnsoLearningBackend/Archivos_u/eventos/";
if($_FILES['archivo']){
    ...
    if ($error > 0){
      ...
      if(move_uploaded_file($archivos_tmp_Name,$upload_name)){
        $response =array(
            "status" => "success",
            "error" => false,
            "menssage" => "eventos subido correctamente",
        ...
        $response = array(
            "status" => "error",
            "error" => true,
            "menssage" => "Error al cargar el eventos!"
        );
      }
    }
}else{
