...
$response = array();
$upload_dir = $_SERVER['DOCUMENT_ROOT']."/api/EnsoLearningBackend/Archivos_u/Portadas_lib/";
$server_url = $_servidor."/api/EnsoLearningBackend/Archivos_u/Portadas_lib/";
if($_FILES['archivo']){
    ...
    if ($error > 0){
      ...
      if(move_uploaded_file($archivos_tmp_Name,$upload_name)){
        $response =array(
            "status" => "success",
            "error" => false,
            "menssage" => "libro subido correctamente",
        ...
        $response = array(
            "status" => "error",
            "error" => true,
            "menssage" => "Error al cargar el libro!"
        );
      }
    }
}else{
